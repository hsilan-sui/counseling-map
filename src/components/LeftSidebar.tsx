          ...
          return (
            ...
            >
              <div className="flex items-center gap-2">
                <span className={`inline-block w-2.5 h-2.5 rounded-full ${c.has_quota ? "bg-green-500" : "bg-gray-400"}`} />
                <span className="font-medium line-clamp-1">{c.org_name}</span>
              </div>
              <div className="text-[11px] text-gray-500 line-clamp-1">
                {c.address}
                {typeof (c as any).distance === "number" && <span> ・{(c as any).distance.toFixed(1)} km</span>}
              </div>
            </li>
          );
        })}
